...
def logsumexp(logits):
    ...
    logsumexp = torch.log(torch.sum(torch.exp(logits - c), axis=1, keepdim=True))
    return c, logsumexp
...
def cross_entropy_loss(logits: torch.Tensor, labels: torch.Tensor):
    ...
    softmax = torch.divide(torch.exp(logits), torch.sum(torch.exp(logits), axis=1)[:, np.newaxis])
    # apply the log-sum-exp strick
    c, lse = logsumexp(logits)
    log_softmax = (logits - c) - lse
    # one-hot encode the labels
    onehot_labels = torch.nn.functional.one_hot(labels.flatten(),num_classes=logits.shape[1])
    
    # implement cross entropy loss
    sel_log_softmax = onehot_labels * log_softmax
    loss = -torch.mean(torch.sum(sel_log_softmax, axis=1))
    return loss
